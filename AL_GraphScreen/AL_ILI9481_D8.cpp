...
const static uint8_t soMA[] PROGMEM =
    {
        // AL_SO_LANDSCAPE1 = 0
        MA_ROW_COL_EX | MA_ROW_ORDER,

        // AL_SO_LANDSCAPE2 = 1
        MA_ROW_COL_EX | MA_COL_ORDER,

        // AL_SO_PORTRAIT1 = 2
        0,

        // AL_SO_PORTRAIT2 = 3
        MA_ROW_ORDER | MA_COL_ORDER};

#ifdef AL_GS_RECOVER_PIN_BEFORE_TRANSFER
#define RECOVER_PIN               \
    {                             \
...
uint32_t AL_ILI9481_D8::readStatus()
{
    uint8_t x;
    uint32_t res = 0;
...
uint8_t AL_ILI9481_D8::readPowerMode()
{
    uint8_t x;
...
void AL_ILI9481_D8::clear(AL_RgbColor color)
{
    displayOff();
...
void AL_ILI9481_D8::changeOrientation()
{
    RECOVER_PIN;
...
#define WRITE_CHAR_PATTERN(pi, fcb1, fcb2, bcb1, bcb2)             \
    {                                                              \
        uint8_t pattern[fontBytesPerChar];                         \
        for (uint8_t i = 0; i < fontBytesPerChar; i++)             \
        {                                                          \
            pattern[i] = pgm_read_byte_near(fontData + pi + i);    \
        }                                                          \
        uint8_t mask = 1;                                          \
        for (uint8_t ppi = 0; ppi < charHeight; ppi++, mask <<= 1) \
        {                                                          \
            for (uint8_t off = 0; off < fontBytesPerChar; off++)   \
            {                                                      \
...
#define WRITE_CHAR_PATTERN(pi, fcb1, fcb2, bcb1, bcb2)                \
    {                                                                 \
        ...
        {                                                             \
            ...
            uint8_t mask = 0x80;                                      \
            for (uint8_t ppi = 0; ppi < charWidth; ppi++, mask >>= 1) \
            {                                                         \
                if ((pb & mask) != 0)                                 \
                {                                                     \
                    WRITE_BYTE(fcb1);                                 \
    ...
    const char *str, uint8_t maxLen)
{
    ...
    for (uint8_t i = 0; i < maxLen && str[i] != 0; i++, x += charWidth)
    {
        setUpdateArea(x, y, charWidth, charHeight);
        uint16_t pi = ((uint8_t)str[i]) * fontBytesPerChar;
...
void AL_ILI9481_D8::setUpdateArea(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
    BEGIN_CMD;
